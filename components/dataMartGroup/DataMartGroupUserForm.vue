<template>
  <div class="bg-white rounded-lg p-4 shadow-custom-lg">
    <ElCollapse v-model="memberCollapse">
...
const memberCollapse = ref(["1"]);

watch(dataMartGroupStore.members.users, (value) => {
  dataMartGroupStore.save.userIds = _useMap(value, "userId");
});
...
const nonGroupUsers = computed(() =>
  _useMap(
    _useDifferenceBy(
      ...
      dataMartGroupStore.members.users,
      "userId",
    ),
...
const updateMembers = (userId: string) => {
  ...
  if (selectedMember) {
    dataMartGroupStore.members.users.push(selectedMember);
  }
};

const deleteMember = (index: number) => {
  dataMartGroupStore.members.users.splice(index, 1);
};
</script>
